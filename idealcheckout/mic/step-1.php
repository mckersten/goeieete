<?php
	...
	$aFormValues['db_type'] = ((version_compare(PHP_VERSION, '5.3', '>') && function_exists('mysqli_connect')) ? 'mysqli' : 'mysql');
	$aFormValues['db_success'] = '';
	$aFormValues['db_error'] = '';
	$aFormValues['backend_email'] = '';
	$aFormValues['backend_pass'] = '';
	$aFormValues['license'] = '';

	$aFormValues['software_url'] = ic_getRootUrl(2);
	...
	$bIdealcheckoutCurlVerificationError = ic_getCurlVerificationError();
	...
	// MySQL Server/Host
	$aSettings[\'host\'] = \'' . ic_escapeQuotes($aFormValues['db_host'] . ($aFormValues['db_port'] ? ':' . $aFormValues['db_port'] : '')) . '\';

	// MySQL Username
	$aSettings[\'user\'] = \'' . ic_escapeQuotes($aFormValues['db_user']) . '\';

	// MySQL Password
	$aSettings[\'pass\'] = \'' . ic_escapeQuotes($aFormValues['db_pass']) . '\';

	// MySQL Database name
	$aSettings[\'name\'] = \'' . ic_escapeQuotes($aFormValues['db_name']) . '\';

	// MySQL Table Prefix
	$aSettings[\'prefix\'] = \'' . ic_escapeQuotes($aFormValues['db_prefix']) . '\';

	// MySQL Library (MySQL or MySQLi)
	$aSettings[\'type\'] = \'' . ic_escapeQuotes($aFormValues['db_type']) . '\';
...
?' . '>';
		...
		if(!empty($_POST['backend_email']) && !empty($_POST['backend_pass']))
		{
			$aFormValues['backend_email'] = $_POST['backend_email'];
			$aFormValues['backend_pass'] = $_POST['backend_pass'];
		}
		else
		{
			$bUsercreationError = true;
		}
		
		if(!empty($_POST['license']))
		{
			$aFormValues['license'] = $_POST['license'];
		}
		else
		{
			$bLicenseError = true;
	...
	if(sizeof($_POST) && !in_array(true, array($bServerError, $bFtpError, $bDatabaseError, $bUsercreationError)) && !in_array(true, $aFormErrors))
	{
...
?' . '>';
		...
		if(!IDEALCHECKOUT_INSTALL::setFile('/idealcheckout/configuration/install.php', $sFileData, $aFormValues))
		{
			...
			ic_output('Kan het bestand /idealcheckout/configuration/install.php niet aanmaken. Controleer de schrijfrechten op de map /idealcheckout/configuration/');
		}
		...
		if($aFormValues['backend_email'] && $aFormValues['backend_pass'])
		{
			// Save credentials
			if(!IDEALCHECKOUT_INSTALL::createUser($aFormValues))
			{
				ic_output('Cannot create user for the IC backend!');
			}			
		}		
		
		
		if($aFormValues['license'])
		{
			// Save license
			if(!IDEALCHECKOUT_INSTALL::saveLicense($aFormValues))
			{
				ic_output('Cannot save the license, please contact iDEAL Checkout support!');
			}			
		}
		...
		{
			ic_output($aInstallResult[1]);
		}
		elseif(is_bool($aInstallResult) && !$aInstallResult)
		{
			ic_output('Er is een fout opgetreden bij de installatie van de scripts.');
		}
		elseif(is_string($aInstallResult))
		{
			ic_output($aInstallResult);
		}
	...
	{
		if(strpos(ic_getRootUrl(), 'https://') === false)
		{
			$sHtml .= '
	<tr>
		<td colspan="3"><div class="warning">De versie van OpenSSL op uw webserver is mogelijk gevoelig voor de <a href="http://www.heartbleed.com">The Heartbleed Bug</a>.<br>Laat uw server beheerder controleren of OpenSSL up to date is!</div></td>
	</tr>';
		}
		else
		{
			$sHtml .= '
	<tr>
		<td colspan="3"><div class="error">De versie van OpenSSL op uw webserver is mogelijk gevoelig voor de <a href="http://www.heartbleed.com">The Heartbleed Bug</a>.<br>Laat uw server beheerder controleren of OpenSSL up to date is!</div></td>
	</tr>';
		}
	}
	...
	<tr>
		...
		{
			$sHtml .= 'Waar mogelijk proberen we de gegevens voor u op te zoeken in het configuratie bestand van uw webshop: ' . $sSoftwareConfigFile . '.<br><br>';
		}
	...
	<tr>
		<td colspan="3"><h1>IC backend account</h1></td>
	</tr>
	<tr>
		<td colspan="3"><p>In de iDEAL Checkout plugins zit een beheeromgeving waar tools aanwezig zijn en statistieken over transacties in te zien zijn. Hiervoor moet een account worden aangemaakt. De gegevens worden versleuteld opgeslagen, dus kunnen niet uitgelezen worden.</p></td>
	</tr>
	<tr ' . ($bUsercreationError ? 'class="error"' : '') . '>
		<td><b>Email adres</b> <em>*</em></td>
		<td><input name="backend_email" type="text" value="' . htmlentities($aFormValues['backend_email']) . '"></td>
		<td>&nbsp;</td>
	</tr>
	<tr ' . ($bUsercreationError ? 'class="error"' : '') . '>
		<td><b>Wachtwoord</b> <em>*</em></td>
		<td><input name="backend_pass" type="password" value="' . htmlentities($aFormValues['backend_pass']) . '"></td>
		<td>&nbsp;</td>
	</tr>
	<tr>
		<td>&nbsp;</td>
	...
	<tr>
		<td colspan="3"><h1>Licentie</h1></td>
	</tr>
	<tr>
		<td colspan="3"><p>De standaard licentie is: CK4hnmcAqvwtcX6eHF7TzVddWm42M8V4<br>Dit is een FREE to use license en wordt u aangeboden door iDEAL Checkout, hiermee kan u nieuws ophalen, build informatie en nog veel meer!</p></td>
	</tr>
	<tr>
		<td><b>Licentie</b> <em>*</em></td>
		<td><input name="license" type="text" value="CK4hnmcAqvwtcX6eHF7TzVddWm42M8V4"></td>
		<td>&nbsp;</td>
	</tr>
	<tr>
		<td colspan="3"><hr size="1"></td>
