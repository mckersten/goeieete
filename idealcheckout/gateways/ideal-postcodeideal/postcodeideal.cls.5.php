<?php
	...
	/*
		...
		Suitable for:
		Postcode iDEAL - iDEAL Professional
	...
	*/
	...
	class IdealRequest
	{
		...
		public function setAquirer($sAquirerName, $bTestMode = false)
		{
			...
			if(strpos($sAquirerName, 'postcode') !== false) // Postcode
			{
				if($bTestMode)
				{
					$this->sPublicCertificateFile = 'postcode-test.cer';
				}
				else
				{
					$this->sPublicCertificateFile = 'postcode-live.cer';
				}
				
				$this->sAquirerUrl = 'ssl://ideal' . ($bTestMode ? '-test' : '') . '.postcode.nl:443/ideal';
			}
			...
			{
				$this->setError('Unknown aquirer. Please use "Postcode" or "Simulator".', false, __FILE__, __LINE__);
				return false;
			}
		}
