			...
			else
			{
				...
				if($this->getRecordByOrder())
				{
					...
					else
					{
						...
						$oIssuerRequest->setMerchant(trim($this->aSettings['MERCHANT_ID']), '0');
			...
			else
			{
				...
				if($this->getRecordByOrder())
				{
					...
					else
					{
						...
						$oTransactionRequest->setMerchant(trim($this->aSettings['MERCHANT_ID']), '0');
			...
			else
			{
				...
				if($this->getRecordByTransaction())
				{
					...
					else
					{
						...
						$oStatusRequest->setMerchant(trim($this->aSettings['MERCHANT_ID']), '0');
		...
		public function doValidate()
		{
			...
			if(idealcheckout_database_num_rows($oRecordset))
			{
				while($oRecord = idealcheckout_database_fetch_assoc($oRecordset))
				{
					...
					if(function_exists('idealcheckout_update_order_status'))
					{
						idealcheckout_update_order_status($oRecord, 'doReport');
					}
				}
