			...
			else
			{
				...
				{					
					...
					else
					{
						...
						$oOmnikassa->setPaymentMethod('bancontact');
						$oOmnikassa->setCachePath($this->aSettings['TEMP_PATH']);
		...
		public function doReport()
		{
			...
			else
			{
				...
				if(!empty($aPostData['authentication']) && !empty($aPostData['expiry']) && !empty($aPostData['eventName']) && !empty($aPostData['poiId']) && !empty($aPostData['signature']))
				{
					...
					$sAuthenticationString = $aPostData['authentication'];
					...
					// Check signature before its processed
					$sHashString = $sAuthenticationString . ',' . $sExpiryString . ',' . $sEventnameString . ',' . $sPoiIdString;
					$sHash = hash_hmac('sha512', $sHashString, base64_decode($this->aSettings['SIGNING_KEY']));
			
					if(!strcasecmp($sSignatureString, $sHash) === 0)
					{
						$sHtml .= '<p>Invalid notification request.</p>';
					}			
					...
					do
					{
						...
						if(!empty($aResult))
						{
							...
							$sMoreOrders = $aResult['moreOrderResultsAvailable'];	
							...
							foreach($aResult['orderResults'] as $aTransaction)
							{
								...
								// Calculate Hash
								$sOrderString = $sMerchantOrderId . ',' . $aTransaction['omnikassaOrderId'] . ',' . $aTransaction['poiId'] . ',' . $aTransaction['orderStatus'] . ',' . $aTransaction['orderStatusDateTime'] . ',' . $aTransaction['errorCode'] . ',' . $aTransaction['paidAmount']['currency'] . ',' . $aTransaction['paidAmount']['amount'] . ',' . $aTransaction['totalAmount']['currency'] . ',' . $aTransaction['totalAmount']['amount'];

								$sHashString = $sMoreOrders . ',' . $sOrderString;
								$sHash = hash_hmac('sha512', $sHashString, base64_decode($this->aSettings['SIGNING_KEY']));
							...
							$bNextOrder = !empty($aResult['moreOrderResultsAvailable']);
						}
						else
						{
							idealcheckout_log($sResponse, __FILE__, __LINE__);
							idealcheckout_log('Error while unserialising data.', __FILE__, __LINE__);
							return false;
