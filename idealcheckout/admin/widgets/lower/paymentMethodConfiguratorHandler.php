...
require_once(IDEALCHECKOUT_PATH . '/includes/init.php');


$sRecievedInfo = $_POST['resultData'];

//echo $sRecievedInfo;

$sGateway = $_POST['gatewayData'];
...
//echo $sGateway;
	...
	$iFound = array_search($sGateway, array_column($aGateWays, 'code'));
	$sFoundFiltered = $aGateWays[$iFound]['type'];

	//echo var_export($sFoundFiltered);
	
	$sFile = $sFoundFiltered . '.php';
	
	unlink(IDEALCHECKOUT_PATH . '/configuration/' . $sFile);
	
	$handle = fopen(IDEALCHECKOUT_PATH . '/configuration/' . $sFile, 'w');
	fwrite($handle, $sRecievedInfo);

	echo fopen(IDEALCHECKOUT_PATH . '/configuration/' . $sFile, 'r');
?>