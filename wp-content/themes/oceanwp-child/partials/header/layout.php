...
// ?> 
<link href="<?php echo get_stylesheet_directory_uri(); ?>/mobile-slider.css" rel="stylesheet" type="text/css"> 
<?php
do_action('ocean_before_header');

// If transparent header style
if ('transparent' == $header_style || ( 'full_screen' == $header_style && true == get_theme_mod('ocean_full_screen_header_transparent', false) ) || ( 'center' == $header_style && true == get_theme_mod('ocean_center_header_transparent', false) ) || ( 'medium' == $header_style && true == get_theme_mod('ocean_medium_header_transparent', false) )) {
    ?>
    ...
    <header id="site-header" class="<?php echo esc_attr(oceanwp_header_classes()); ?>" data-height="<?php echo esc_attr($header_height); ?>"<?php oceanwp_schema_markup('header'); ?>>
        ...
        if (!function_exists('elementor_theme_do_location') || !elementor_theme_do_location('header')) {
            ...
            <?php
            $menu_childs = get_child_menu_from_parent();
            $post_id = get_the_ID();

            /* echo "<pre>"; print_r($menu_childs);   */
            ?>

            <?php if (!empty($menu_childs) || is_product()){?>   
            <div class="line ">
                <ul>
                    <?php
                    if (!empty($menu_childs)) {
						
                        foreach ($menu_childs as $menu_child) {
							
                            $child_active = ($post_id == $menu_child['object_id']) ? 'child-active' : '';
                            ?>
                            <li>
                                <a href="<?php echo $menu_child['url']; ?>" class="<?php echo $child_active; ?>"><?php
                                    echo $menu_child['title'];
                                    ?>
                    ...
                    <?php if (is_product()) { ?>
                        <li>
                            <i class="fa fa-th" aria-hidden="true"></i> <a href="<?php echo site_url('bestellen'); ?>"><span>Terug naar overzicht</a></span>
                        </li>
                        <li>
