<?php
	...
	class woocommerce_idealcheckout_mastercard extends WC_Payment_Gateway
	{
		public function __construct()
		{
			global $woocommerce;

			$this->id = 'idealcheckout_mastercard';
			$this->method_title = 'iDEAL Checkout - Mastercard';
			$this->icon = plugins_url('assets/images/mastercard.png', __FILE__);
		...
		public function init_form_fields()
		{
			$this->form_fields = array(
				'enabled' => array(
					...
					'type' => 'checkbox',
					'label' => idealcheckout_getTranslation($this->getLanguageCode(), 'woocommerce', 'Enable Mastercard Payment'),
					'default' => 'no'
				),
				'title' => array(
		...
		{
?>
<h3><?php echo idealcheckout_getTranslation($this->getLanguageCode(), 'woocommerce', 'iDEAL Checkout - Mastercard'); ?></h3>
...
`transaction_log` = NULL;";
	...
	function woocommerce_add_idealcheckout_mastercard($methods) 
	{
		$methods[] = 'woocommerce_idealcheckout_mastercard'; 
		return $methods;
	}

	add_filter('woocommerce_payment_gateways', 'woocommerce_add_idealcheckout_mastercard');
